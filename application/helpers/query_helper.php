...
//        create list
    ...
    public static function record_dependency_check($table_name, $field_name, $field_value)
    {
        $CI =& get_instance();
        $result = Query_helper::get_info($table_name, 'id', array( $field_name.' = '.$field_value, 'status = ' . $CI->config->item('STATUS_ACTIVE') ), 1);
        if(!empty($result)){
            return true;
        } else {
            return false;
    ...
    public static function get_max_ordering( $table_name ){
        $CI =& get_instance();
        $CI->db->select_max('ordering');
        $query = $CI->db->get($table_name)->row_array();
