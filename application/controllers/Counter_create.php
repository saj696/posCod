 ...
 * Time: 4:18 PM
 */
class Counter_create extends Root_Controller
{
    public $permissions;
    ...
    function __construct()
    {
        ...
        $this->model_name = "counter_create_model";
    ...
    {
        $data['task_title']=$this->lang->line('COUNTER_LIST');
        $data['new_button_name']=$this->lang->line('COUNTER_CREATE');
        $this->template->load('admin_default_template', 'counter_create/record_list',$data);
    }

    public function get_record_list()
    {
        //$model=$this->model_name;
        $record_list=Query_helper::get_info($this->config->item('table_counters'),"id, counter_name, CASE
    ...
    public function add_record()
    {
        ...
        if($this->input->post())
        {
            if(!$this->check_validation())
            {
                ...
                $data['task_title']=$this->lang->line('COUNTER_CREATE');
                $this->template->load('admin_default_template', 'counter_create/add_record', $data);
            }
            else
            {
                ...
                $record_data['counter_name'] = $input_data['counter_name'];
                ...
                Query_helper::add($this->config->item('table_counters'), $record_data);
    ...
    public function edit_record($record_id)
    {
        ...
        if($this->input->post())
        {
            ...
            if(!$this->check_validation($record_id))
            {
                ...
                $data['record_info']=Query_helper::get_info($this->config->item('table_counters'),'*',array('id = '.$record_id),1);
                $data['task_title']=$this->lang->line('COUNTER_EDIT');
                $this->template->load('admin_default_template', 'counter_create/edit_record', $data);
            }
            else
            {
                $user_id=User_helper::get_user()->id;
                ...
                Query_helper::update($this->config->item('table_counters'), $record_data, array('id ='.$record_id));
    ...
    private function check_validation($record_id='')
    {
        ...
        if(empty($record_id))
        {
            $user=Query_helper::get_info($this->config->item('table_counters'), '*',array("counter_name = '".$this->input->post('counter_name')."'"), 1);
            if(!empty($user))
            {
        ...
        $this->form_validation->set_rules('counter_name',$this->lang->line('NAME'),'required');
